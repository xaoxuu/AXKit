...
static inline CALayer *sCAMaskLayerWithSizeAndCorner(CGSize size, CGFloat cornerRadius){
...
static inline UIColor *lighten(UIColor *color){
    CGFloat red = 0.0,green = 0.0,blue = 0.0, alpha = 1.0;
    [color getRed:&red green:&green blue:&blue alpha:&alpha];
...
static inline UIColor *defaultColor(){
...
static inline UIView *getStatusBarProgressMessageContentView(){
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (([UIScreen instancesRespondToSelector:@selector(currentMode)] ? CGSizeEqualToSize(CGSizeMake(1125, 2436), [[UIScreen mainScreen] currentMode].size) : NO)) {
            ...
            view.frame = frame;
            view.layer.mask = sCAMaskLayerWithSizeAndCorner(view.bounds.size, 0.5*height);
        } else {
        ...
        progressLayer.backgroundColor = lighten(defaultColor()).CGColor;
...
static inline void updateStatusBarProgress(CGFloat progress){
    ...
    if (!color) {
        color = lighten(defaultColor());
    }
    progressLayer.backgroundColor = darkRatio(color, 0.2).CGColor;
...
static inline void showStatusBarMessageView(NSTimeInterval duration){
    ...
    // 超时自动消失
    static dispatch_block_t timeoutToken;
    !timeoutToken?:dispatch_block_cancel(timeoutToken);
    timeoutToken = dispatch_block_create(0, ^{
        hideStatusBarMessage();
    });
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(duration * NSEC_PER_SEC)), dispatch_get_main_queue(), timeoutToken);
...
static inline UILabel *getStatusBarMessageLabel(NSString *text){
    ...
    if (offset > 0) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.38f * NSEC_PER_SEC)), dispatch_get_main_queue(), timeoutToken);
